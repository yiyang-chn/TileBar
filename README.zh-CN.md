...
平铺分两步：**权重决定矩形大小**，**你当前的窗口位置决定哪个窗口去哪个矩形**。所以如果默认平铺把 Slack 放左边、Claude 放右边而你更习惯反过来，把它们拖到自己想要的位置再按平铺即可——布局会跟着你的空间偏好走。Chrome 还是会比 Terminal 大（权重），但每个 app 摆在哪是你说了算（位置）。

同一显示器内的匹配会最小化全部窗口的位移总和。
