...
平铺完成后，拖任意一个窗口的边缘会实时联动到相邻的窗口：拖 Chrome 的右边缘 → 右边的 Claude/Slack 同步变窄，保持布局无缝。拖 Claude 的底边 → Slack 的顶边跟着移动。底层用 AX resize 通知，所以是跟手实时刷新的（顽固应用比如腾讯 QQ 仍然不响应 AX setSize，这是已知问题）。下次平铺或撤销时联动会话自动结束。
