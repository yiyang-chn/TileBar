...
窗口按**当前空间顺序**（上到下、左到右）喂给 squarify。每个窗口的槽位长在你拖到的地方，大小按它的权重算——把重 app 拖到左边，左边的槽就跟着变大。所以如果默认平铺把 Slack 放左边、Claude 放右边而你更习惯反过来，把它们拖到想要的位置再按平铺即可。权重决定**多大**，你的拖动决定**在哪**。
