...
平铺完成后，拖任何一条**分隔线**上的任意窗口边缘，所有共用这条线的窗口都会实时跟着动。拖 Chrome 的右边缘，或者拖右上窗口的左边缘 — 同一条线，同样的效果：Chrome 变窄，整个右列（上下两个窗口）一起变宽，布局保持无缝。拖 Claude 的底边、或者拖 Slack 的顶边 — 它们之间的水平分隔线，都会跟着你的手走。底层用 AX resize 通知，跟手实时刷新。

已知限制：腾讯 QQ 等少数顽固应用即使用了 `AXEnhancedUserInterface` 这套标准 workaround 也仍然不响应 AX `setSize`，它们不会被联动 resize；如果你拖的是窗口标题栏（整窗平移，不是边缘）会破坏布局——重新平铺一次清理即可。下次平铺或撤销时联动会话自动结束。

### 布局决策

两个独立信号：

- **位置** —— 你的拖动决定每个窗口落到哪个槽位。排序使用带容差的空间分组（横屏 column-major、竖屏 row-major）：两个窗口的中心点如果落在主轴上同一个 100px 带内，就被视为同列/同行，再按上下（或左右）顺序排。所以你不需要把两个窗口在像素级对齐，"右侧上下两个"这种意图能被准确识别。
- **大小** —— 槽位面积来自 [TileBar/ContentMeasurer.swift](TileBar/ContentMeasurer.swift) 里的每应用静态权重。Chrome（2.2）和 Claude/Slack 一起平铺时大约占 16:9 屏幕的 53%，与平铺前任何窗口实际大小无关。**拖动只影响位置，不影响大小**。要改每应用的大小比例，去改这个权重表。
